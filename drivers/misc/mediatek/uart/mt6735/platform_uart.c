...
//hebiao@wind-mobi.com 20160605 begin
#ifdef CONFIG_MTK_CHARGER_RED_DETECTC_OUT_HIGH
	if (uartport == 2) {
		printk("GPIO57_UART=%s fun==%s line=%d\n", uart_gpio_cmds[uartport][gpioopid],  __func__, __LINE__);
		return;
	}else {
	pinctrl_select_state(ppinctrl, pins_uart);
	}
#endif
//hebiao@wind-mobi.com 20160605 end
