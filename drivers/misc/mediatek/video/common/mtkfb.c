...
//dingyisheng@wind-mobi.com 20161122 begin
char tp_lcm_name[126]={0};
//dingyisheng@wind-mobi.com 20161122 end
/**
...
static int __parse_tag_videolfb(unsigned long node)
{
	...
	if (videolfb_tag) {
		...
		//dingyisheng@wind-mobi.com 20161122 begin
		memset((void *)tp_lcm_name, 0, sizeof(tp_lcm_name));
		strcpy((char *)tp_lcm_name, videolfb_tag->lcmname);
		tp_lcm_name[strlen(videolfb_tag->lcmname)] = '\0';
		//dingyisheng@wind-mobi.com 20161122 end
...
int _parse_tag_videolfb(void)
{
	...
	if (of_scan_flat_dt(fb_early_init_dt_get_chosen, &node) > 0) {
		...
		if (!ret) {
			...
			printk("[DT][videolfb] lcmfound=%d, fps=%d, fb_base=%p, vram=%d, lcmname=%s,tp_lcm_name=%s\n",
			     islcmconnected, lcd_fps, (void *)fb_base, vramsize, mtkfb_lcm_name,tp_lcm_name);
			strncmp(tp_lcm_name, mtkfb_lcm_name, 100);
			return 0;
		}

		printk("[DT][videolfb] videolfb_tag not found\n");
		return 1;
	}
