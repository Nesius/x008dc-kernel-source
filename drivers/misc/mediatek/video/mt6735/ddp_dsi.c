...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 5; i++) { //moified by hebiao at 2016824 zhangkaiyuan
			if (dsi_params->lcm_esd_check_table[i].cmd == 0) {
				//printk("read_kong dsi_params->lcm_esd_check_table[%d].cmd=%d\n", i, dsi_params->lcm_esd_check_table[i].cmd);
				break;
			}
			...
			/* / 0xB0 is used to distinguish DCS cmd or Gerneric cmd, is that Right??? */
			//printk("t0.data0 cmd is %x \n",t0.Data0);  //liupeng
			t0.Data_ID =(t0.Data0 <0xff) ? DSI_DCS_READ_PACKET_ID :DSI_GERNERIC_READ_LONG_PACKET_ID;//liupeng modify B0 to 00
			t0.Data1 = 0;
			t1.CONFG = 0x00;
		...
		for (i = 0; i < 5; i++) { //moified by hebiao at 2016824 zhangkaiyuan
			if (dsi_params->lcm_esd_check_table[i].cmd == 0) {
				//printk("cmp_kong dsi_params->lcm_esd_check_table[%d].cmd=%d\n", i, dsi_params->lcm_esd_check_table[i].cmd);
				break;
			}
		//	DISPCHECK("[DSI]enter cmp i=%d\n", i);
			...
			if (hSlot[0]) {//liupeng
				...
				printk("esd read hslot now~~4007 \n");
				if (i == 0) {
			...
			printk("twz [DSI]enter cmp read_data1 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x \n",read_data1.byte0,read_data1.byte1,read_data1.byte2,read_data1.byte3);
			printk("twz [DSI]enter cmp read_data2 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x \n",read_data2.byte0,read_data2.byte1,read_data2.byte2,read_data2.byte3);
			printk("twz [DSI]enter cmp read_data3 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x \n",read_data3.byte0,read_data3.byte1,read_data3.byte2,read_data3.byte3);
			printk("twz [DSI]short enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x,para_list[2]=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd,dsi_params->lcm_esd_check_table[i].count,dsi_params->lcm_esd_check_table[i].para_list[0],dsi_params->lcm_esd_check_table[i].para_list[1]
			,dsi_params->lcm_esd_check_table[i].para_list[2]);
			printk("twz [DSI]long enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x,para_list[2]=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd,dsi_params->lcm_esd_check_table[i].count,dsi_params->lcm_esd_check_table[i].para_list[0],dsi_params->lcm_esd_check_table[i].para_list[1]
			,dsi_params->lcm_esd_check_table[i].para_list[2]);
			printk("twz[DSI]enter cmp DSI+0x200=0x%x\n",AS_UINT32(DDP_REG_BASE_DSI0+0x200));
			printk("twz [DSI]enter cmp DSI+0x204=0x%x\n",AS_UINT32(DDP_REG_BASE_DSI0+0x204));
			printk("twz [DSI]enter cmp DSI+0x60=0x%x\n",AS_UINT32(DDP_REG_BASE_DSI0+0x60));
			printk("twz [DSI]enter cmp DSI+0x74=0x%x\n",AS_UINT32(DDP_REG_BASE_DSI0+0x74));
			printk("twz [DSI]enter cmp DSI+0x88=0x%x\n",AS_UINT32(DDP_REG_BASE_DSI0+0x88));
			printk("twz [DSI]enter cmp DSI+0x0c=0x%x\n",AS_UINT32(DDP_REG_BASE_DSI0+0x0c));
#endif			
			packet_type = read_data0.byte0;
			...
			{
			// printk("DSI read long packet data exceeds buffer size: %d\n", dsi_params->lcm_esd_check_table[i].count);
			 recv_data_cnt = dsi_params->lcm_esd_check_table[i].count;
			}
			printk("DSI read long packet size: %d\n", recv_data_cnt);
//hebiao@wind-mobi.com 20160322 begin
	switch(recv_data_cnt)
		...
		case 1:
			if((read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0]) )
				{
   	//		printk("tuwz 3 byte");
				//printk("esd_check: %#x, %#x, %#x, %#x, %#x, %#x, \n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]
					//, read_data1.byte2,dsi_params->lcm_esd_check_table[i].para_list[2]);
				ret = 0; // esd pass
				//printk("hebiao func==%s line=%d ret1=%d\n", __func__, __LINE__, ret);
				}
				else
				{		
				//printk("esd_check1: %#x, %#x, %#x, %#x, %#x, %#x \n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]
					//, read_data1.byte2,dsi_params->lcm_esd_check_table[i].para_list[2]);
				ret = 1; // esd fail
				//printk("hebiao func==%s line=%d ret2=%d esd fail \n", __func__, __LINE__, ret);
				
				goto lable;
				}
				break;
		case 2:
				if((read_data1.byte0==0x05&&read_data1.byte1==0x18)||((read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0]) && (read_data1.byte1 == dsi_params->lcm_esd_check_table[i].para_list[1])))
				{
   	//		printk("tuwz 3 byte");
				//printk("esd_check: %#x, %#x, %#x, %#x, %#x, %#x, \n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]
					//, read_data1.byte2,dsi_params->lcm_esd_check_table[i].para_list[2]);
				...
				ret = 0; // esd pass
				//printk("hebiao func==%s line=%d ret3=%d\n", __func__, __LINE__, ret);
				}
				else
				{
				//printk("esd_check1: %#x, %#x, %#x, %#x, %#x, %#x \n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]
					//, read_data1.byte2,dsi_params->lcm_esd_check_table[i].para_list[2]);
				ret = 1; // esd fail
				//printk("hebiao func==%s line=%d ret4=%d esd fail \n", __func__, __LINE__, ret);
				goto lable;
				}
				break;
		case 3:
   	...
   	//		printk("tuwz 3 byte");
					...
					ret = 0; // esd pass
					//printk("hebiao func==%s line=%d ret5=%d\n", __func__, __LINE__, ret);
				}
				else
				{
				//printk("esd_check1: %#x, %#x, %#x, %#x, %#x, %#x \n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]
					//, read_data1.byte2,dsi_params->lcm_esd_check_table[i].para_list[2]);
				ret = 1; // esd fail
				//printk("hebiao func==%s line=%d ret6=%d esd fail\n", __func__, __LINE__, ret);
				goto lable;
				}
				break;
//hebiao@wind-mobi.com 2016824 begin
		case 10:
				if((read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0]) && (read_data1.byte1 == dsi_params->lcm_esd_check_table[i].para_list[1]) 
   	...
   	//		printk("tuwz 3 byte");
   	#if 0
					printk("esd_check: %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x, %#x,%#x, %#x\n", 
					read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1],
					read_data1.byte2, dsi_params->lcm_esd_check_table[i].para_list[2], read_data1.byte3, dsi_params->lcm_esd_check_table[i].para_list[3],
					read_data2.byte0, dsi_params->lcm_esd_check_table[i].para_list[4], read_data2.byte1, dsi_params->lcm_esd_check_table[i].para_list[5],
					read_data2.byte2, dsi_params->lcm_esd_check_table[i].para_list[6], read_data2.byte3, dsi_params->lcm_esd_check_table[i].para_list[7],
				...
				break;
//hebiao@wind-mobi.com 2016824 end
	default:
			break;
			}  //switch case
//hebiao@wind-mobi.com 20160322 end		
	}else {
		//printk("hebiao func==%s line=%d other type\n", __func__, __LINE__);
		recv_data_cnt = 2;
		if(recv_data_cnt > (dsi_params->lcm_esd_check_table[i].count))
		{
	...
	//		printk("tuwz 1 byte");
				...
				//printk("hebiao func==%s line=%d ret7=%d\n", __func__, __LINE__, ret);
				ret = 0; // esd pass
			}
			else
			{
				ret = 1; // esd fail
				//printk("hebiao func==%s line=%d ret8=%d\n", __func__, __LINE__, ret);
				break;
			}
		}
		}
		...
		/* create 3 slot */
//hebiao@wind-mobi.com 2016824 begin zhangkaiyuan
		cmdqBackupAllocateSlot(&hSlot[0], 5);
		cmdqBackupAllocateSlot(&hSlot[1], 5);
		cmdqBackupAllocateSlot(&hSlot[2], 5);
		cmdqBackupAllocateSlot(&hSlot[3], 5);
//hebiao@wind-mobi.com 2016824 end
	...
	lable:  //liupeng@wind-mobi.com 20160315 end
	return ret;
}
