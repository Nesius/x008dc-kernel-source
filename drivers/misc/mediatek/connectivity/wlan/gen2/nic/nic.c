...
WLAN_STATUS nicUpdateTxPowerOffset(IN P_ADAPTER_T prAdapter, IN P_CMD_MITIGATED_PWR_OFFSET_T prTxPwrOffsetParam)
{
	...
	/* Print previous 4 sets as INFO for debug */
	for (i = 0 ; i < 4 ; i++) {
		DBGLOG(INIT, INFO, "%s: RlmMitigatedPwrByChByMode (%d)(%d)(%d)(%d)(%d)\n",
			__func__,
			prTxPwrOffsetParam->arRlmMitigatedPwrByChByMode[i].channel,
			prTxPwrOffsetParam->arRlmMitigatedPwrByChByMode[i].mitigatedCckDsss,
			prTxPwrOffsetParam->arRlmMitigatedPwrByChByMode[i].mitigatedOfdm,
			prTxPwrOffsetParam->arRlmMitigatedPwrByChByMode[i].mitigatedHt20,
			prTxPwrOffsetParam->arRlmMitigatedPwrByChByMode[i].mitigatedHt40);
	}

	/* Print remaining part as TRACE for detail debug */
	for (i = 4 ; i < 40 ; i++) {
		DBGLOG(INIT, TRACE, "%s: RlmMitigatedPwrByChByMode (%d)(%d)(%d)(%d)(%d)\n",
			__func__,
			prTxPwrOffsetParam->arRlmMitigatedPwrByChByMode[i].channel,
			prTxPwrOffsetParam->arRlmMitigatedPwrByChByMode[i].mitigatedCckDsss,
			prTxPwrOffsetParam->arRlmMitigatedPwrByChByMode[i].mitigatedOfdm,
